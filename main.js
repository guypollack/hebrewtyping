function turnGreen(letter) {

    if (translateToHebrew(letter) === allLetters[0].innerHTML) {
        document.getElementById(`${lettersCount-allLetters.length+1}`).style.backgroundColor = "aquamarine";
        allLetters.shift();
    } else {
        if (letter != " ") {
            mistakes++;
            document.getElementById("mistakes").innerHTML = `Mistakes: ${mistakes}`;
            mistakesDict[allLetters[0].innerHTML]++
        }
    }
...
function showHideEnglish() {
    const setting = document.getElementById("show-english").checked === true ? "visible" : "hidden";
    console.log(setting);
...
document.body.addEventListener("keypress", function(event) {
    ...
    //console.log(event.key,translateToHebrew(event.key));
...
function findAllowedLetters() {
    let allowedLetters = [];
    for (box of document.querySelectorAll("input[type='checkbox']")) {
        if (box.checked === true) {
...
function findForbiddenLetters() {
    ...
    for (box of document.querySelectorAll(".allowed-letters-boxes input[type='checkbox']")) {
        if (box.checked === false) {
            forbiddenLetters.push(document.getElementById(`${box.id}-label`).innerHTML);
        }
    }
...
function createRandomString() {
    allowedLetters = findAllowedLetters();
...
function generateSentence() {
    ...
    allLetters = Array.from(document.querySelectorAll(".letter"));
